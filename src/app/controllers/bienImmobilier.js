...
router.delete("/remove/:id", authMiddleware, function(req, res) {
    BienImmobilier.deleteOne({ _id: req.params.id, userId: req.user._id },
        function(err) {
            if (err) {
...
router.put("/update-selected-annoncement/:id", authMiddleware, (req, res) => {
    if (req.user.role === "agent") {
        BienImmobilier.findOneAndUpdate({ _id: req.params.id, agentId: req.user._id }, {...req.body },
            (err, data) => {
                if (err)
                    res
                    .status(400)
        ...
        BienImmobilier.findOneAndUpdate({ _id: req.params.id, userId: req.user._id }, {...req.body, situation: false },
            (err, data) => {
                if (err)
                    res
                    .status(400)
